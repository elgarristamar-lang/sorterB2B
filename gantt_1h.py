...
def export_visual_sheet(
    ...
    for i, sub in enumerate(subramps, start=header_row + 1):
        ...
        for j, hm in enumerate(hour_minutes, start=2):
            ...
            is_especial = "ESPECIAL" in full.upper()
            cell = ws.cell(row=i, column=j, value=visible)
            cell.alignment = Alignment(horizontal="center", vertical="center", wrap_text=True)
            cell.font = Font(bold=is_especial, size=9 if is_especial else 8)
    ...
    # Merge por tramos — key = playa text only, ignoring used/cap and multi-block variation
    for i in range(header_row + 1, header_row + 1 + len(subramps)):
        ...
        for j in range(2, 2 + len(hour_minutes) + 1):
            if j <= 1 + len(hour_minutes):
                ...
                # Use the stable playa text as merge key (not the label with counts)
                cur_key = str(m["full"]) if m else None
            else:
                cur_key = None
            ...
            if run_key and run_start is not None:
                ...
                if run_end > run_start:
                    ...
                    # Re-apply bold for especial after merge (merge can reset formatting)
                    anchor = ws.cell(row=i, column=run_start)
                    if "ESPECIAL" in str(run_key).upper():
                        anchor.font = Font(bold=True, size=9)
